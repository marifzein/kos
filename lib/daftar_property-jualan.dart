...
//POPUP
  ...
  loopBuild() {
    ...
    if (byk > 0)
      for (int i = 0; i < byk; i++) {
        //for (int i = 0; i < propertiTemp.length; i++) {
        log('i = $i');
        return listBuild(
            //propertiTemp[i]['id'] as int,
            propertiTemp[i]['id'].toString(),
            propertiTemp[i]['nama'].toString(),
            propertiTemp[i]['alamat'].toString() +
                '-' +
      ...
      log('blm ada data');
      return listBuild(
          "0", 'Anda belum input data properti milik anda', 'klik tombol +');
    }
  }

  listBuild(String propertiId, String namaKost, String alamat) {
                      ...
                      ElevatedButton(
                        ...
                        onPressed: () {
                          ...
                          showDialog(
                              context: context,
                              builder: (BuildContext context) {
                                return properti(namaKos: namaKost);
                              });
                          //END OF POPUP
                        },
                        child: Icon(
